...
export default function AddPayment({ onClose }) {
  ...
  const { data: tenantData, isLoading: tenantLoading } = useGetallactivetenantQuery(selectedBranch, {
    skip: !selectedBranch
  });

  const [createPayment, { isLoading: paymentLoading,isSuccess }] = useCreatePaymentMutation();
  ...
  useEffect(() => {
    ...
    if (tenantData?.findAllTenant) {console.log(tenantData?.findAllTenant); setTenants(tenantData.findAllTenant)};
    if(isSuccess){
        navigate(-1)
    }
  }, [branchData, tenantData,isSuccess]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.branch || !formData.tenantId) {
      alert("Please select both Branch and Tenant.");
      return;
    }
 ...
 return (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center p-4 z-50">
    <div className="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-6 border border-white/30 
      animate-scaleIn">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white shadow-sm
              focus:ring-4 focus:ring-[#1e3a5f]/30 transition-all"
            required
          >
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white shadow-sm
              focus:ring-4 focus:ring-[#1e3a5f]/30 transition-all disabled:bg-gray-100"
            disabled={!selectedBranch || tenantLoading}
            required
          >
            <option value="">{tenantLoading ? "Loading..." : "Choose tenant..."}</option>
            {tenants.map((tenant) => (
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button
            ...
            className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#1e3a5f] to-[#3b5b96] text-white font-semibold
